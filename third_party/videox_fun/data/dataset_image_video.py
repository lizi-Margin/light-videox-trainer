...
def _read_video_from_dir(video_dir):
    frames = []
    for frame_path in sorted(list(glob.glob(os.path.join(video_dir, '*.png')))):
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image') == 'video' and data_info.get('mask_path', None) is None:
            ...
            if not self.enable_bucket:
                ...
                pixel_values = torch.cat([input_video, target_video, mask_video], dim=1)
                pixel_values = self.video_transforms(pixel_values)
                input_video = pixel_values[:, :3]
                target_video = pixel_values[:, 3:6]
                mask_video = pixel_values[:, 6:]
